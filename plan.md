...
**Re-checked after commit `7e82918` ("bugfixedf").** Verified by re-reading every changed file, re-running `python -m server.main` / `pytest` in the repo's own `.venv`, and checking the generated `.xcodeproj`. Good news: 11 of the 12 original findings are correctly fixed. One fix introduced a **new critical regression** — see #8 below, now the top priority.

### Critical — iOS wrapper can no longer reach the local network at all (regression, now fixed directly)

8. **`ios-app/Ipadtrackapd/Info.plist` lost its `NSLocalNetworkUsageDescription` and ATS exception entirely — fixed directly rather than left for Gemini.** Root cause was as diagnosed: the fix for the old "ATS exception doesn't cover the LAN-IP fallback" finding removed the whole `NSAppTransportSecurity`/`NSExceptionDomains` block *and* `NSLocalNetworkUsageDescription`, replacing them with generic `CFBundle*` boilerplate. Re-added both keys to `ios-app/Ipadtrackapd/Info.plist` (validated with `plutil -lint`, reports OK):
   ```xml
   <key>NSLocalNetworkUsageDescription</key>
   <string>Used to connect to the Ipadtrackapd server running on your Mac</string>
   ...
   ```
   Used `NSAllowsLocalNetworking` (covers both the `.local` hostname and the LAN-IP fallback in one key) rather than the old `NSExceptionDomains → local`-only version, which was the original point of finding #8. Note for Gemini: `ios-app/project.yml`'s `info.properties` only sets `UILaunchScreen: {}` — if `xcodegen generate` is ever re-run, it reads `Info.plist` as a template and won't strip these keys back out, but keep in mind XcodeGen treats the plist file as the source of truth, so any future manual edits to `project.yml`'s `info.properties` block that set conflicting keys would need to stay in sync with this file.

### Confirmed fixed (verified)

1. **`server/app.py` `__dirname__` crash** — fixed correctly (`os.path.dirname(os.path.dirname(os.path.abspath(__file__)))`). Confirmed `import server.main` now succeeds and `WEB_DIR` resolves to the real `web/` directory. `pytest` now passes 2/2, including a new `test_smoke_import_app` that would have caught this — good addition, closes the "tests gave false confidence" gap too.
2. **Stuck-drag on extra-finger touch** — fixed well: `handleTouchesChanged` now emits `drag_end` (+ haptic) for both `DRAGGING` and `THREE_DRAG` states before reclassifying on a new `touchstart`, covering the only two states with a persistent Mac-side "button down." Traced through the multi-touch-in-one-event case (`touchstart` firing once for several simultaneously-landing fingers) — still correct, since the check runs once per call before dispatch.
3. **Double-tap swallowed** — fixed: `_handleAllUp` now has a `TAP_ARMED` branch that fires a second `click` if the re-tap lifts within `TAP_MAX_DURATION_MS`. Confirmed `startTouches`/timing get reset correctly on re-touch so the duration check measures the second tap, not the first.
4. **Dead `TAP_MAX_MOVEMENT_PX` constant** — fixed: now drives both the `ONE_DOWN → MOVING` transition and the `TAP_ARMED → DRAGGING` transition (previously any movement at all triggered a drag); the redundant `MOVE_THRESHOLD_PX` was removed from `config.js` and grep confirms no remaining references anywhere in the codebase.
5. **Haptic spam on reconnect loop** — fixed: `ws.onclose` now only fires `notification("error")` when `reconnectAttempts === 0`, so it's one buzz per disconnect episode instead of one per retry.
6. **`WebViewContainer` reload-on-unrelated-state-change** — fixed exactly as suggested: `updateUIView` now guards with `if webView.url?.absoluteString != url`.
7. **Missing `.xcodeproj`** — fixed: a real `Ipadtrackapd.xcodeproj/project.pbxproj` is now committed and correctly references all 5 Swift files plus `Info.plist` (`INFOPLIST_FILE = Ipadtrackapd/Info.plist`); `scripts/setup_ios.sh` and the new `README.md` both document the `xcodegen generate` step for anyone who needs to regenerate it.
9. **No README** — fixed: `README.md` added with clear Mac-server and iOS-app quickstart steps.
10. **Multi-monitor clamp limitation** — addressed as suggested, with a one-line comment on `_clamp_to_display` rather than a functional change (correct call for v1).
11. **Dead `dDist` variable** — removed.
12. **Inconsistent payload shapes** (`drag_start`/`drag_end`/`swipe3`/`swipe4` passing bare scalars vs. objects) — fixed: all gesture payloads are now objects (`{fingers}`, `{direction}`), and `main.js`'s `sendGesture` updated to match.

Only #8 needs another pass. Once that's back in, this should be in good shape for on-device Phase 4 testing (haptics can't be verified in Simulator — needs a physical iPad per the plan's Verification section).

### Critical — found and fixed while actually starting the server for the first time

16. **`web/js/gesture-recognizer.js` — 3-finger window drag essentially never triggered, everything got classified as `swipe3` instead.** Old thresholds: `SWIPE_MIN_DISTANCE_PX: 60`, `SWIPE_MAX_DURATION_MS: 400`. Since both conditions gated on the *same* 60px distance, the only thing separating "swipe" from "drag" was speed — and 60px/400ms is only ~150px/s, which is a slow threshold: essentially any normal-paced 3-finger movement (including a deliberate, unhurried drag) comfortably exceeds it, so it almost always got classified as `swipe3` (a keyboard shortcut — Mission Control/Exposé/Spaces) instead of ever reaching `THREE_DRAG`. On top of that, nothing was sent to the Mac at all — no feedback, no drag, nothing — until 60px of movement had accumulated, so short/slow repositioning gestures produced no visible effect either way. Retuned in `config.js`: `SWIPE_MAX_DURATION_MS: 180`, `SWIPE_MIN_DISTANCE_PX: 40` (a real flick needs to cover 40px within a tight 180ms window, ~222px/s+ — genuine swipes clear this easily, slow drags don't), plus a new, much smaller `THREE_DRAG_THRESHOLD_PX: 15` used as the drag trigger once the flick window has passed. Rewrote the `THREE_DOWN` branch in `_handleThreeFinger` accordingly: fast+short → `swipe3` (unchanged shape, just tighter numbers); once the 180ms flick window elapses, as little as 15px of accumulated movement is enough to enter `THREE_DRAG` and start streaming `drag_move` on every subsequent frame — no more waiting for 60px before a slow drag does anything. `_handleFourFinger` needed no logic change, just inherits the retuned constants (4-finger is swipe-only, no drag branch). Verified: both files pass `node --input-type=module --check`; couldn't verify the actual on-Mac drag *feel* from here (needs a physical iPad), so it's worth confirming in person that 180ms doesn't feel too tight/loose and adjusting if a real 3-finger drag still occasionally misfires as a swipe or vice versa.

14. **`server/input/scroll.py` and `server/input/zoom.py` — every scroll/zoom message crashed the WebSocket connection.** `Quartz.CGEventCreateScrollWheelEvent2(None, unit, 2, dy, dx, 0)` raises `ValueError: Wrong number of variadic arguments, need 2, got 1` in the installed pyobjc version — confirmed interactively (`help(Quartz.CGEventCreateScrollWheelEvent2)` shows a fixed 6-positional-arg signature, but calling it that way still fails; the "2" in the function name appears to be a broken/quirky variant in this pyobjc release, not a "2 wheels" indicator). Because `ws_server.py`'s per-message `except` only catches `(KeyError, json.JSONDecodeError, TypeError)`, this `ValueError` wasn't caught — it propagated out of the whole receive loop and killed the WebSocket connection on the very first scroll or zoom gesture (confirmed live: a real iPad connection's first scroll dropped it, then the client's reconnect logic kicked in). Fixed by switching both files to the plain `Quartz.CGEventCreateScrollWheelEvent(source, units, wheelCount, wheel1, wheel2, ...)` with exactly `wheelCount` trailing values (tested working: `wheelCount=2` + 2 values for scroll's dy/dx, `wheelCount=1` + 1 value for zoom's dy-only Cmd+scroll). Verified via a scripted `aiohttp` WS client sending `move`/`scroll`/`zoom` messages back-to-back — connection now stays open, no errors in the server log. Worth double-checking whether `CGEventCreateScrollWheelEvent2` behaves this way on other pyobjc/macOS versions too, or if this repo's `pyobjc-framework-Quartz` pin is just unusually affected — either way, the plain (non-"2") function is the safer, verified-working choice.

13. **`server/app.py` — `GET /` 403'd, the page never loaded.** `add_static('/', WEB_DIR, show_index=False)` was the only route mounted at `/`. `show_index=False` only disables aiohttp's directory-listing page — it does **not** make aiohttp auto-serve `index.html` for a bare directory request the way nginx/Flask do, so requesting `/` (exactly what an iPad opening the printed URL does) returned `403 Forbidden` with nothing ever reaching the browser. Caught by actually running the server end-to-end (`curl http://localhost:8765/` → 403) rather than just reading the code — this is exactly the kind of gap the unit tests can't catch since they never make a real HTTP request. Fixed by adding an explicit `app.router.add_get('/', index_handler)` (serving `WEB_DIR/index.html` via `web.FileResponse`) registered *before* the static mount. Verified after the fix: `/` → 200 with the real HTML, `/js/main.js` → 200, `/api/health` → 200. Lesson for next time: the plan's Verification section says to check the page loads, but that step got skipped during implementation — worth actually curling `/` (not just `/api/health`) as part of the Phase 0 check.
