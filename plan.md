...
Reviewed the current implementation against this plan by reading every file and actually running the server/tests in the repo's own `.venv`. Findings below, most severe first. Items marked "verified" were confirmed by running code, not just reading it.

### Critical — the server cannot start at all (verified)

1. **`server/app.py:6`** — `WEB_DIR = os.path.abspath(os.path.join(os.path.dirname(__dirname__), "web"))` uses `__dirname__`, which doesn't exist in Python (that's a Node.js global). Confirmed by running `python -m server.main` in the project's own `.venv`:
   ```
   NameError: name '__dirname__' is not defined. Did you mean: '__name__'?
   ```
   This crashes on import, so the app never gets past startup — nothing in the current build actually runs. Fix: `WEB_DIR = os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "web"))` (two `dirname` calls: `server/app.py` → `server/` → repo root → `web`).
   Note: `tests/test_ws_dispatch.py` passes today (`1 passed`) purely because it only imports `server.ws_server`/`server.input.backend`, never `server.app` or `server.main` — so the test suite currently gives false confidence and didn't catch this. Worth adding a trivial smoke test that imports `server.main`/`server.app` so this class of bug fails CI next time.

### High — reachable functional bugs

2. **`web/js/gesture-recognizer.js` — a stray extra finger touch can leave the Mac's mouse button stuck down.** `handleTouchesChanged` re-runs `_startSession(newState, touches)` on *every* `touchstart`, regardless of `this.locked` (the condition is `if (!this.locked || eventType === "start")`, and `eventType` is `"start"` on every new finger touching down). This is needed for progressive 1→2→3-finger session buildup, but it also means a gesture that's already locked and in flight gets silently reclassified the moment an *extra* finger touches down — there's no guard limiting this to the initial buildup. Concretely: user does a one-finger tap-drag (`state = DRAGGING`, Mac's `mouse.py` has posted `LeftMouseDown` and set `_dragging = True`), a second finger brushes the glass → `_startSession(TWO_DOWN, ...)` overwrites `state` to `TWO_DOWN` → `_handleAllUp` later never sees `state === DRAGGING`, so `drag_end` is never sent → the Mac's left mouse button is never released (`force_release_all()` only runs on WebSocket close, not on this in-session reclassification). Same issue applies to `THREE_DRAG`. This directly contradicts the plan's own stated design ("the gesture type is locked once classified... finger-count decreases mid-session ends the gesture rather than reinterpreting"), which only accounted for decreases, not increases. Fix: while `state` is `DRAGGING` or `THREE_DRAG` (or really any state past the initial buildup), an increase in finger count should either be ignored (extra fingers don't retrigger `_startSession`) or should force-emit the matching `*_end` before reclassifying.

3. **Double-tap is silently swallowed.** After a confirmed tap, `state` becomes `TAP_ARMED` and a `DRAG_ARM_WINDOW_MS` timer starts. If the user taps again quickly *without* moving (an intentional double-tap, not a drag), `_handleAllUp` runs again but none of its `if/else if` branches match `TAP_ARMED` (only `ONE_DOWN`, `DRAGGING`, `TWO_DOWN`, `THREE_DRAG` are handled), and the trailing `if (this.state !== STATE.TAP_ARMED) { this.state = STATE.IDLE; }` also skips resetting because state *is* `TAP_ARMED`. Net result: the second tap produces no `click` at all, and `state` stays latched on `TAP_ARMED` until the original timer fires. Add a branch in `_handleAllUp` for `state === STATE.TAP_ARMED` that fires a second `click` (+ haptic) when the re-tap lifts without crossing the drag-move threshold.

4. **`CONFIG.TAP_MAX_MOVEMENT_PX` (8px) is dead — never read anywhere in `gesture-recognizer.js`.** Tap-vs-move is actually decided entirely by `MOVE_THRESHOLD_PX` (4px), which drives the `ONE_DOWN → MOVING` transition in `_handleOneFinger`. Since 4px is smaller than the intended 8px tap tolerance, ordinary finger jitter (5-8px, very common on a real tap) already flips state to `MOVING` before lift, so `_handleAllUp`'s `state === STATE.ONE_DOWN` check fails and no `click` fires — the tap turns into a tiny, likely-unnoticed cursor drift instead. Either use `TAP_MAX_MOVEMENT_PX` (not `MOVE_THRESHOLD_PX`) as the gate for whether a lift counts as a tap in `_handleAllUp`, or consciously drop the unused constant from `config.js` if `MOVE_THRESHOLD_PX` is meant to be the only threshold — right now the code has two overlapping thresholds and only honors one.

### Medium

5. **`web/js/main.js` — error haptic spams during reconnect.** `ws.onclose` unconditionally calls `haptics.notification("error")` before scheduling a reconnect. This fires on the very first failed connection attempt (server not up yet) and again on every failed retry in the backoff loop — i.e. a repeating "error" buzz every 500ms–5s for as long as the Mac server is unreachable. Only notify once on the transition from connected → disconnected (track a `wasConnected` flag, or only call it if `reconnectAttempts === 0` right before incrementing).

6. **`ios-app/Ipadtrackapd/WebViewContainer.swift` — `updateUIView` reloads the page on unrelated SwiftUI state changes.** It unconditionally calls `webView.load(request)` every time `updateUIView` runs, and SwiftUI calls `updateUIView` on *every* re-render of `ContentView`, not just when `url` changes — e.g. tapping the gear icon (`showingSettings = true`) triggers a reload, killing the live WebSocket session. Guard it: `if webView.url?.absoluteString != url { webView.load(request) }`.

7. **`ios-app/` has no actual Xcode project, only `project.yml` (an XcodeGen spec).** There is no `.xcodeproj` in the repo, and `xcodegen` is not installed on this machine (`which xcodegen` → not found). The plan's install instructions ("open `ios-app/Ipadtrackapd.xcodeproj` in Xcode") don't work as written — that file doesn't exist yet. Either commit a generated `.xcodeproj` directly, or add a documented step (README + ideally a `scripts/`/Makefile helper) for `brew install xcodegen && (cd ios-app && xcodegen generate)` before opening in Xcode.

8. **`Info.plist`'s ATS exception only covers the `local` domain, not the LAN-IP fallback.** `discovery.py` prints, and `SettingsView`'s own footer text explicitly tells the user to try, a plain IP-literal fallback URL (e.g. `http://192.168.1.5:8765`) for when `.local` resolution fails. That's a different ATS case than the `NSExceptionDomains → local` entry currently covers and may be silently blocked. Add `NSAllowsLocalNetworking: true` (simplest fix — covers private-range IPs, loopback, and `.local` in one key) so the documented fallback path actually works.

### Low / cleanup

9. **No `README.md`** exists anywhere in the repo, though the plan's file tree calls for one. Add basic run instructions (Mac server via `scripts/run.sh`; iOS app via Xcode/XcodeGen per #7) so someone can get started without re-reading all of `plan.md`.
10. **`server/input/mouse.py`'s `_clamp_to_display`** only clamps against `CGDisplayBounds(CGMainDisplayID())`, so on a multi-monitor setup the cursor can never be driven onto a secondary display positioned beside the main one. Fine for v1 single-display use; worth a one-line comment noting the limitation.
11. **`web/js/gesture-recognizer.js`'s `_handleTwoFinger`** computes `const dDist = dist - this.prevDistance;` but never uses it — dead variable, safe to delete.
12. **`web/js/main.js`'s `sendGesture`** mixes payload shapes per message type — `drag_start`/`drag_end`/`swipe3`/`swipe4` pass a bare scalar (finger count or direction string) as `payload` while `move`/`scroll`/`zoom`/`drag_move` pass an object. It works today only because `gesture-recognizer.js` and `main.js` happen to agree on the shape for each type, but it's a fragile, easy-to-break convention. Consider standardizing on an object payload for every gesture type.
